...
//			int result = 100 / zero;
		...
		private void InitParametersDropDown()
		{
			...
			LocationLetter.SelectedIndex = 0;
			foreach (var n in Numbers) {
				LocationNumber.Items.Add(new ListItem(n.ToString()));
			}
			LocationNumber.SelectedIndex = 0;
		}
...
//				RiskName.Text = ss.SourceName;
			...
			else if (componentType.Equals("Problem"))
			{
				...
				if (author.Length == 2) {
					...
					var resultString = Regex.Match(problem.Author, @"\d+").Value;

					RiskAuthorFIN.Text = resultString;

					int index = problem.Author.IndexOf(resultString);
					string cleanPath = (index < 0)
						? problem.Author
						: problem.Author.Remove(index, resultString.Length);
					RiskAuthor.Text = cleanPath;
				}
				...
				if (riskLocation.Length == 2) {
					...
					var id1 = Util.GetHtmlSelectIdForDepartment (problemFacet.LocationDetail);
					if (id1 > -1)
						RiskDepartment.SelectedIndex = id1;
				}
